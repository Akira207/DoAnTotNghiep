...
export default function MaterialsHistoryTable({ items = [] }) {
  if (!items.length) {
    return (
  ...
  const getTypeClass = (type) => {
    ...
    if (lower.includes("phụ") || lower.includes("kiện")) return "bg-secondary-container text-on-secondary-container";
    if (lower.includes("sơn") || lower.includes("hóa chất")) return "bg-tertiary-container text-on-tertiary-container";
    ...
    <div className="bg-surface-container-lowest rounded-xl shadow-sm overflow-hidden border border-slate-100">
              ...
              >
                <td className="px-6 py-5 whitespace-nowrap">
                  <div className="flex flex-col">
                    <span className="text-sm font-bold text-on-surface">
                      {new Date(item.createdAt || new Date()).toLocaleDateString('vi-VN')}
                    </span>
                    <span className="text-[10px] text-slate-400 font-bold uppercase">
                      {new Date(item.createdAt || new Date()).toLocaleTimeString('vi-VN', { hour: '2-digit', minute: '2-digit' })}
                    </span>
                  </div>
                </td>
                ...
                <td className="px-6 py-5 whitespace-nowrap">
                  <span className="text-sm font-semibold text-on-surface">
                    {item.materialName || item.name || "Vật liệu"}
                  </span>
                </td>

                <td className="px-6 py-5 whitespace-nowrap">
                  <span
                    className={`px-2.5 py-1 ${getTypeClass(item.category)} text-[10px] font-black rounded tracking-wide uppercase`}
                  >
                    {item.category || "Khác"}
                  </span>
                </td>
                ...
                <td className="px-6 py-5 whitespace-nowrap">
                  <div className="flex items-center gap-2">
                    <div className="w-6 h-6 rounded bg-slate-100 flex items-center justify-center text-[10px] font-bold text-slate-400">
                      {(item.supplier || "?").substring(0, 2).toUpperCase()}
                    </div>
      ...
      <div className="px-6 py-4 bg-white flex items-center justify-between border-t border-slate-100">
        <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">
          Hiển thị {items.length} trong số {items.length} kết quả
        </p>
